...
export const helloWorld = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if(data.type=="PAGE_VIEW" && data.page_view){
      ...
      const utm = data.page_view.utm_campaign  && data.page_view.utm_source ? {
        utm_campaign:{
    ...
    if(data.type==="PAGE_EXIT"||data.type==="PATH_CHANGE" && data.path_changes){
      const page_history = (data.path_changes||[]).map((e)=>({page:e, visits:1}))
await update_analytics_cache(
        {
       pages:page_history
      },
