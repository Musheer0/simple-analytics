...
const OtherAnalytics = ({data}:{data:WebsiteAnalytics}) => {
 ...
 // Compute colors in JavaScript for dark/light theme
  ...
  // Prepare device data
  const deviceData = Object.entries(data.devices).map(([key, value]: [string, any]) => ({
    name: key,
    value: value,
  }))

  // Prepare browser data
  const browserData = Object.entries(data.browsers).map(([key, value]: [string, any]) => ({
    name: key,
    value: value,
  }))
  ...
  const utmCampaignData = Object.entries(data.utm_campaign||{}).map(([key, value]: [string, any]) => ({
    name: key,
    value: value,
  }))
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={pageData}>
                ...
                <Tooltip
                  ...
                  cursor={{ fill: theme === 'dark' ? 'rgba(59, 130, 246, 0.1)' : 'rgba(37, 99, 235, 0.1)' }}
                />
                <Bar dataKey="visits" fill={chartColors.primary} radius={[8, 8, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={utmSourceData}>
                ...
                />
                <Bar dataKey="visits" fill={chartColors.secondary} radius={[8, 8, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card className="border-border lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Breakdown of browsers and operating systems</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                ...
                <div className="space-y-2">
                  {Object.entries(data.browsers).map(([browser, count]: [string, any]) => (
              ...
              <div>
                ...
                <div className="space-y-2">
                  {Object.entries(data.operating_systems).map(([os, count]: [string, any]) => (
