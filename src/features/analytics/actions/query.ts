...
export const getBasicWebsiteAnalytics = async ({
  ...
  const cacheKey = redisKeys[`BASIC_WEBSITE_ANALYTICS_${duration}`](website.id);

  const cache = await redis.get<WebsiteAnalytics>(cacheKey);
  const days = Math.round(
    ANALYTICS_TIME_MS[duration] / ANALYTICS_TIME_MS.ONE_DAY,
  );
  if (cache) {
    const trend = await getTrendAnalytics({
  ...
  const [summary, utm, metadata, pages, avgSession] = await Promise.all([
