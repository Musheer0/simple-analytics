...
const page = async({params,searchParams}:{params:Promise<{id:string}>,searchParams:Promise<{range:keyof typeof ANALYTICS_TIME }>}) => {
  const {id} = await params;
  const {range} = await searchParams
  const analytics = await getBasicWebsiteAnalytics({websiteId:id,duration:range ||"ONE_WEEK",includeWebsite:true})
